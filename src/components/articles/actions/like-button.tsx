...
import { toggleLike } from "@/src/actions/like-toggle";
...
const LikeButton: React.FC<LikeButtonProps> = ({
  ...
  const handleLike = async () => {
    ...
    startTransition(async () => {
      ...
      if (result?.error) {
        ...
        setOptimisticLikes(likes.length);
        alert(result.error);
      }
    });
  };
  ...
  if (showSignInPrompt && !isLoggedIn) {
    return (
      <div className="flex flex-col sm:flex-row items-center gap-4 p-4 bg-muted rounded-lg border-t pt-8 mb-12">
